...
function drawGoombaLine() {
  // Generate a random number of goombas in the line (5, 7, 10)
  var numGoombas = difficulty + Math.floor(Math.random() * 3);
  // Section the path into equal length segments
  var sectionPathWidth = Math.floor( 83 / numGoombas );  // we only want to use one quarter of the canvas width so that the goombas are generated near the middle of the screen

  // Holds the unique x positions for the goombas
  var positions = [];
